...
export const [TitleSelectorProvider, useTitleSelectorStore] = defineStore<TitleSelectorStore>(
  (set, get) => ({
    selectedTitles: [],
    actions: {
      ...
      unselectTitles(titles: IndexedTitleV2[]) {
        const filteredTitles = get().selectedTitles.filter(
          ({ uuid }) => !titles.some((title) => title.uuid === uuid),
        );
        set({ selectedTitles: filteredTitles });
      },
      clearSelectedTitles() {
...
export const titleSelectorSelector = {
  ...
  isTitleSelected(title: IndexedTitleV2) {
    return (state: TitleSelectorStore) =>
      state.selectedTitles.some(({ uuid }) => title.uuid === uuid);
  },
